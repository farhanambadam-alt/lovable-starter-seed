...
const deleteBranchSchema = z.object({
  owner: ownerField,
  repo: repoField,
  branch: z.string().min(1, 'Branch name required').max(255),
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { owner, repo, branch, provider_token } = deleteBranchSchema.parse(body);
    ...
    if (profile.github_username !== owner) {
      return new Response(
        JSON.stringify({ error: 'You can only delete branches from your own repositories' }),
        { status: 403, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }

    console.log(`Deleting branch: ${branch} from ${owner}/${repo}`);
